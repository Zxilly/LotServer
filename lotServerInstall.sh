...
function Install()
{
  ...
  URLKernel='https://github.com/Zxilly/LotServer/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/Zxilly/LotServer/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/Zxilly/lotServer/raw/master/lotServer.tar"
  tar -xvf "/tmp/lotServer.tar" -C /tmp
  acce_ver=$(acce_check ${KNV})
  # 如果有自己搭建的或者api失效，这里修改成你自己的api即可
  wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "https://api.learningman.top/keygen/keygen.php?ver=${acce_ver}&mac=${Mac}"
